...
public class CubeTransformer implements PageTransformer {
	 ...
	 * 当一个页面填充整个屏幕是，它的值是0，当一个页面刚刚离开屏幕的右边时，它的值是1。当两个也
	 * 页面分别滚动到一半时，其中一个页面的位置是-0.5，另一个页面的位置是0.5。基于屏幕上页面的位置
	 ，通过使用诸如setAlpha()、setTranslationX()、或setScaleY()方法来设置页面的属性，来创建
	 自定义的滑动动画。
     */
